...
class Builder extends CoreObject {
  ...
  setupBroccoliBuilder() {
    ...
    let broccoli, options = {}, systemTemp = isExperimentEnabled('SYSTEM_TEMP');
    if (isExperimentEnabled('BROCCOLI_2')) {
      ...
      try {
        ...
        console.log('');
        console.warn(chalk.red('ERROR: Invalid Broccoli2 node detected, falling back to broccoli-builder. Broccoli error:'));
    ...
    if (systemTemp) {
      console.warn(chalk.red('EMBER_CLI_SYSTEM_TEMP only works in combination with EMBER_CLI_BROCCOLI_2'));
    }
