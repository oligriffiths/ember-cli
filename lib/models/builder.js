...
class Builder extends CoreObject {
  ...
  constructor(options) {
    ...
    this.broccoli2 = Boolean(options.broccoli2 || experiments.BROCCOLI_2);
    this.systemTemp = Boolean(options.systemTemp || experiments.SYSTEM_TEMP);
  ...
  */
  ...
  build(willReadStringDir, resultAnnotation) {
    ...
    return this.processAddonBuildSteps('preBuild')
      ...
      .then(this.compatNode.bind(this))
   ...
   */
  compatNode(node) {
    if (!this.broccoli2 || !(node instanceof broccoli2nodeClass)) {
      return node;
    }
